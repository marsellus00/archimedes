...
export async function GET(request: NextRequest) {
  try {
    ...
    if (projectId) {
      ...
      return NextResponse.json({
        status: "phase_5_project_audit_ready",
        mode: "project_chat",
    ...
    return NextResponse.json({
      status: "phase_5_free_chat_audit_ready",
      mode: "free_chat",
    ...
    if (isApiError(error)) {
      return NextResponse.json(
        { status: "phase_5_audit_error", code: error.code, error: error.message },
