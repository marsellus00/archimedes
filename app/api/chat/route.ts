...
export async function GET() {
  ...
  return NextResponse.json({
    status: "phase_5_free_chat_ready",
    message:
    ...
    safeBoundary: [
      "Production authentication is handled in Phase 6; Phase 5 consumes trusted auth context or local dev headers.",
      ...
      "Document retrieval remains a Phase 7 pipeline, but file and chunk tables are included in the schema.",
    ],
  });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requireDatabase =
      process.env.REQUIRE_PHASE5_DATABASE === "true" || process.env.NODE_ENV === "production";
    ...
    let requestContext = buildPhase5RequestContext(chatRequest, {
      chatMode,
    ...
    if (!database.configured && requireDatabase) {
      return NextResponse.json(
        {
          error:
            "Phase 5 database integration requires DATABASE_URL in production. Configure PostgreSQL before using persisted chat.",
          status: "phase_5_free_chat_ready",
          database,
        },
        { status: 503 },
...
function buildResponseEnvelope(input: {
  ...
  return {
    ...
    notes: [
      ...
      "Phase 5 persists database-backed free or project chat exchanges when DATABASE_URL is configured.",
      "Full user authentication, MFA, and account security pages remain Phase 6; this route already consumes trusted authenticated context.",
...
function resolveImplementationStatus(
  ...
  if (scopeStatus !== "engineering") {
    return "phase_3_scope_guarded";
  }

  if (runtimeMode === "live_openai_compatible") {
    return "phase_5_database_integrated";
  }
...
function buildPhase5RequestContext(
  chatRequest: EngineeringChatRequest,
  options: Pick<Phase3RequestContext, "persistenceStatus" | "chatMode">,
): Phase3RequestContext {
  return {
    userId: undefined,
    projectId: resolveRequestedProjectId(chatRequest),
    chatMode: options.chatMode,
    authenticationStatus: "deferred_until_auth_phase",
