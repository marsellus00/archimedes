...
export async function GET(request: NextRequest) {
 ...
 if (!session) {
 return NextResponse.json(
 { status: "history_error", error: "Chat session was not found in this project." },
 { status: 404 },
 );
 }

 return NextResponse.json({
 status: "chat_session_ready",
 mode: "project_chat",
 ...
 if (!session) {
 return NextResponse.json(
 { status: "history_error", error: "Free chat session was not found for this user." },
 { status: 404 },
 );
 }
 ...
 if (isApiError(error)) {
 return NextResponse.json(
 { status: "history_error", code: error.code, error: error.message },
