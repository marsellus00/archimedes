...
export async function resolveUserContextFromRequest(
  ...
  if (requestUser.authenticationStatus === "required_but_missing" || !requestUser.email) {
    throw new ApiError(
      401,
      "Authenticated user context is required. Phase 5 expects Phase 6 auth or trusted gateway headers.",
...
export async function getOrCreateDefaultProjectForUser(user: User): Promise<Project> {
  ...
  const project = await prisma.project.create({
    data: {
      ...
      description:
        "Auto-created Phase 5 project used until the user creates a dedicated project.",
      discipline: "General Engineering",
      metadataJson: toJsonSafe({ createdBy: "phase_5_default_project" }),
      members: {
        create: {
          userId: user.id,
          role: "OWNER",
