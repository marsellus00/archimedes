...
export function createMockEngineeringProvider(options?: {
  ...
  const info = {
    providerName: "Deterministic Phase 3 fallback provider",
    ...
    async generateEngineeringResponse(
      ...
      return {
        ...
        warnings: [
          ...
          "This is suitable for Phase 3 route validation and UI integration, not final assistant behavior.",
        ],
...
function resolveFallbackResponse(input: EngineeringAIProviderInput) {
  ...
  return {
    ...response,
    answer: [
      "Phase 3 AI provider path is active and the request passed the engineering-scope gate.",
