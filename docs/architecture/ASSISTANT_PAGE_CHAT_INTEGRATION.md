...
The `/assistant` route is now a focused live chat workspace connected to the AI provider integration `/api/chat` endpoint.
...
- Messages are sent to `POST /api/chat` with a AI provider integration development user header.

## Deferred controls

Attachment, calculation tool, document context, and voice controls are displayed as secondary actions but remain deferred until their future roadmap items. They show a notice instead of pretending to perform unavailable work.

## Boundaries

This UI does not add production authentication, database persistence, document retrieval, or deterministic calculation tools. Those remain scheduled for future roadmap items.
