...
export function DashboardPage() {
  const [phase5Data, setPhase5Data] = useState<DashboardApiResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        ...
        if (!ignore) setPhase5Data(data);
      } catch {
        if (!ignore) {
          setPhase5Data({
            status: "phase_5_dashboard_unavailable",
  ...
  const liveCounts = phase5Data?.dashboard?.counts;
      ...
      <section className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div>
          ...
          <p className="max-w-2xl text-lg leading-8 text-slate-400">
            Engineering workspace for deterministic calculations, live AI chat, documentation, and project traceability. Phase 5 adds PostgreSQL-backed projects, saved chats, saved calculations, audit logs, and live dashboard data.
          </p>
          {phase5Data?.message ? (
        ...
        {[
          ...
          <div key={label} className="rounded-2xl border border-slate-700/30 bg-[#161b22] p-5">
            ...
            <div className="mt-2 text-xs text-slate-500">
              {phase5Data?.project?.name ?? "Default project when database is configured"}
            </div>
          </div>
        ))}
      </section>
